...
script_content = """-- 1. Make columns nullable in class_sessions
...
-- 2. Drop old policies that depend on institution_id
...
-- 3. Drop institution_id from reservations and subscriptions
...
-- 4. Recreate RLS policies for reservations
...
try:
    ...
    # Write script
    stdin, stdout, stderr = ssh.exec_command('cat > /tmp/refactor_schema.sql')
    stdin.write(script_content)
    stdin.channel.shutdown_write()
    print("Write script:", stdout.read().decode(), stderr.read().decode())
    
    # Find docker container name and run script
    # It might be `supabase-db` or `db` depending on docker-compose naming
    cmd = 'docker exec -i supabase-db psql -U postgres -d postgres < /tmp/refactor_schema.sql'
    ...
    if "No such container" in err:
        cmd2 = 'docker ps | grep postgres | grep supabase'
        i2, o2, e2 = ssh.exec_command(cmd2)
        print("Looking for container:", o2.read().decode())
    else:
