...
class _SaasSubscriptionViewState extends State<SaasSubscriptionView> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content:
                  Text('No se pudieron cargar los datos. Intentá nuevamente.')),
        );
      }
    }
  }
  ...
  Future<void> _cancelSubscription() async {
    ...
    try {
      ...
      if (response.status != 200) {
        throw Exception(
            response.data['error'] ?? 'Error desconocido al cancelar.');
      }
  ...
  Widget build(BuildContext context) {
    ...
    final currentPlanName = _currentSubscription != null &&
            _currentSubscription!['saas_plans'] != null
        ? _currentSubscription!['saas_plans']['name']
        : 'Ninguno';
    final currentStatus = _currentSubscription != null
  ...
  Widget _buildInfoBadge(
      String label, String value, KaliColorsExtension kaliColors) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(label, style: kaliColors.caption(kaliColors.clayDark)),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color:
                kaliColors.espresso.withValues(alpha: highlight ? 0.1 : 0.02),
            blurRadius: 16,
